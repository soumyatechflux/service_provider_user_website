...
import UpdateExamModal from "./UpdateFinalExamModal";
...
export default function FinalExamCard({program, finalExam, setEdited,selectedExam,setSelectedExam}) 
{

    console.log("SET EDITED",setEdited)
    console.log("FINAL EXAM CARD",finalExam)
    ...
    const { mutate,isPending  } = useMutation({
        ...
        onMutate: () => {
            ...
            queryClient.setQueryData(['finalExams', program._id], (oldData: unknown) => {
                ...
                const newData = oldData.slice().filter(exam => exam._id !== finalExam._id)
                return newData
            })
        ...
        onSuccess: () => {
            // ✅ Refetch final exams from the server to get updated data
            //@ts-expect-error finalExams
            queryClient.invalidateQueries(["finalExams", program._id]);
            handleClose(); // ✅ Close modal after deletion
        },
        mutationFn: () => setDeleteFinalExam( program, selectedExam)
    ...
    return (
        ...
        >
            ...
            >
                <DialogTitle sx={{ mx: 1, mt: 2, mb: 3 }}>Are you sure you want to delete {finalExam?.examName} Model Exam?</DialogTitle>
                <DialogContent>
                ...
                <Button 
                    ...
                    onClick={() => {
                        mutate()
                        console.log("ID",selectedExam?._id)
                        handleClose()
                    }}
                >
                    Yes
            ...
            {finalExam?.map((exam:FinalExamProps) => {
            console.log("Exam Name:", finalExam);
            return(
                <Box key={exam?._id} mb={2}>
                    ...
                    >
                        <Typography fontSize={14} fontFamily="Inter" fontWeight={500} >
                            {exam?.examName}
                        ...
                        <Typography 
                            ...
                            sx={{cursor:"pointer"}}
                            onClick={() => {
                                setSelectedExam(exam ); // ✅ Select exam
                ...
                />
                    ...
                    >
                        <Stack direction="column" alignItems="center" justifyContent="center" py={5} gap={1}>
                        ...
                        <Stack direction="row" flex={1} gap={5}>
                            ...
                            //  disabled={isPending} 
                                ...
                                // onClick={() => handleClickOpen(finalExam?._id)
                                //     setSelectedExam(exam );
                                // }
                                onClick={() => {
                                    handleClickOpen()// ✅ Select exam
                                    setSelectedExam(exam ); // ✅ Open modal
                            
                                }}
                            >
                                {isPending ? "Deleting..." : "Delete Exam"} {/* ✅ Show loading text */}
                            </Button>
                            <Button
                                sx={{
                                    width: '145px',
                                ...
                                // onClick={() => setEdited(finalExam._id)}
                                 onClick={() =>{
                                    setSelectedExam(exam)
                                    console.log("EDIT CLICK",selectedExam?._id)
                                    setEdited(selectedExam?._id)
                                } }
                            >
                                Edit Exam
                            </Button>
