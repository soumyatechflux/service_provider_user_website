    ...
    const fetchSubCategory = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_SERVICE_PROVIDER_USER_WEBSITE_BASE_API_URL}/api/customer/sub_category/${service.id}`
        );
  
        const data = response.data?.data;
  
        if (data?.view_details) {
          const htmlString = data.view_details;
          const tempElement = document.createElement('div');
          tempElement.innerHTML = htmlString;
  
          const points = Array.from(tempElement.querySelectorAll('li')).map(
            (li) => li.textContent.trim()
          );
  
          setViewDetails(points);
        } else {
          setViewDetails([]);
        }
      } catch (error) {
      ...
      <div className="modal-dialog modal-dialog-scrollable service-detail-modal">
        <div className="modal-content">
          ...
          <div className="modal-body">
            <h6 className="modal-subtitles">Key Details:</h6>
            {/* <ul>
        {viewDetails.map((point, index) => (
          <li key={index}>{point}</li>
        ))}
            </ul> */}
      {
        service && service.view_details ? (
          <div
            className="text-sm text-gray-700"
            dangerouslySetInnerHTML={{ __html: service.view_details }}
          />
        ) : (
